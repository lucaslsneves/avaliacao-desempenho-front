...
export default function HorizontalCard({
  ...
  function handleDelete() {
    ...
    api.put(`/assessments-groups/${assessmentGroupId}`, { active: false }, {
      headers: {
        Authorization: token
      }
    }).then(({ data }) => {
  ...
  return (
    <HStack cursor="pointer" maxWidth={admin === true ? '800px' : '700px'} as="li" height="48" w="100%" borderRadius="lg" bg={useColorModeValue("white", "gray.700")} shadow="lg" >
      ...
      <VStack justifyContent="space-between" alignItems="flex-start" height="100%" padding="5" flex="65" width="100%">
        <VStack alignItems="flex-start" width="100%" position="relative">
          ...
          {admin && (
            <>
              <IconButton
                ...
                position="absolute"
              />
              <Modal scrollBehavior={"inside"} size="md" onClose={onClose} isOpen={isOpen} isCentered>
                <ModalOverlay />
                <ModalContent>
                  ...
                  <ModalBody padding="5" >
                    ...
                    <HStack margin="0 auto" marginTop="5" spacing="5" width="50%" justifyContent="center">
                      ...
                      <Button isLoading={isLoadingButton} onClick={handleDelete} colorScheme="red">Excluir</Button>
                    </HStack>
                  </ModalBody>
          ...
          {
            admin && <HStack justifyContent="space-between" width="100%" >
              <HStack>
          ...
          {admin === true ? (
            <HStack justifyContent="space-between" width="100%" alignItems="center">
             <HStack>
             <Text fontWeight="500">Feedback Colaborador</Text> <Switch defaultIsChecked={availableToSeeCollaborator} onChange={async () => {
               const token = 'Bearer ' + localStorage.getItem('token')
               api.put(`/assessments/${assessmentId}`, { availableToSeeCollaborator: !availableToSeeCollaborator }, {
                 headers: {
                   Authorization: token
                 }
               }).then(({ data }) => {
