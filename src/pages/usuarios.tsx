...
export default function Usuarios() {
  ...
  const [users, setUsers] = React.useState<{ data: Array<any>; meta: {} }>({
    data: [],
    meta: {},
  });
  ...
  function loadUsers(filter = "", page = 1) {
    ...
    const token = "Bearer " + localStorage.getItem("token");
    api
      .get(`/users?filter=${filter}&page=${page}`, {
        headers: {
          Authorization: token,
        },
      })
      ...
      <VStack spacing="8" height="100%" width="100%">
        ...
        >
          ...
          <HStack>
            <Button
              ...
              onClick={() => loadUsers(search, users.meta.current_page - 1)}
        ...
        >
          {users.data.map((user) => (
            ...
            >
              <Text fontWeight="500" width="40%">
                {user.name}
              </Text>
              <Text fontWeight="500" textAlign="justify" width="30%">
                {user.cpf}
              </Text>
              <Text fontWeight="500" textAlign="justify" width="30%">
                {user.role}
              ...
              <IconButton
                ...
                colorScheme="green"
                onClick={() =>
                  myOnOpen(user.id, user.name, user.cpf, user.email, user.role)
                }
                icon={<EditIcon />}
              />
            </HStack>
