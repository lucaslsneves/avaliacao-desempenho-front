...
export default function TodasAvaliacoes(props) {
  ...
  useEffect(() => {
    const token = "Bearer " + localStorage.getItem("token");
    api
      .get(`/assessments-groups?filter=${filter}`, {
        headers: {
          Authorization: token,
        },
      })
      ...
      <VStack spacing={6}>
        ...
        {assessmentsGroups.map((assessmentGroup) => (
          <HorizontalCard
            ...
            availableToSeeCollaborator={
              assessmentGroup.available_to_see_collaborator
            }
            availableToAnswer={assessmentGroup.available_to_answer}
            endDate={format(new Date(assessmentGroup.end_date), "dd/MM/yyyy")}
            startDate={format(
              new Date(assessmentGroup.start_date),
              "dd/MM/yyyy"
            )}
            name={assessmentGroup.name}
            key={assessmentGroup.assessment_group_id}
            handleClick={() => {
              history.push(`/todos-tipos-avaliacoes`, {
